...
import { staticRecipes, filterAndSortStaticRecipes } from "@/data/staticData";
...
const PopularRecipes: React.FC<PopularRecipesProps> = ({ 
  ...
  const { data: recipes, isLoading, error } = useQuery<Recipe[]>({
    ...
    queryFn: async () => {
      // Check if we're in production (static build) environment
      const isStaticBuild = import.meta.env.MODE === 'production';
      
      if (isStaticBuild) {
        // Use static data when deployed on GitHub Pages
        return filterAndSortStaticRecipes(staticRecipes, activeFilters, activeSortOption, limit);
      } else {
        // Use API in development environment
